...
SELECT name, 
	CASE WHEN monthlymaintenance <= 100 THEN 'cheap'
		ELSE 'expensive' END AS 'monthly maintenance'
FROM facilities;
...
FROM 	
	...
	FROM bookings as b
	INNER JOIN facilities AS f USING(facid)
	INNER JOIN members AS m USING(memid) ) AS subq
...
SELECT facName AS 'Facility Name',
...
SELECT CONCAT(m1.firstname," ", m1.surname) as 'Member Name',
		CASE WHEN m1.recommendedby = 0 THEN ''
			ELSE CONCAT(m2.firstname," ", m2.surname) END as 'Recommended By'
FROM members as m1
...
SELECT f.name as Facility,
		...
		b.useCount AS 'Use Count'
FROM (SELECT memid, facid, count(*) as useCount
	  FROM bookings
	  WHERE memid != 0
...
SELECT f.name as Facility,
		b.startmonth AS 'Month',
		b.useCount AS 'Use Count'
FROM (SELECT facid, memid, month(starttime) as startmonth, count(*) as useCount
	  FROM bookings
	  WHERE memid != 0
	  GROUP BY facid, startmonth) as b
INNER JOIN facilities AS f USING(facid)
INNER JOIN members AS m USING(memid)
ORDER BY Facility, b.startmonth;
